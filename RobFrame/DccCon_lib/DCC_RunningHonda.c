...
void fnvCalMstab(double dParas[4]) { // Kp_pit, Kd_pit, Kp_rol, Kd_rol
	...
	Mstab.rol = -Kp_rol * dBaseErr.x - Kd_rol * dBaseErr.dx;
	for (int i = 0; i < 2; i++, nRek++) {
...
void fnvMdzmp(double dParas[4], double dLagTsplit) { // Kp_pos, Kd_pos, Kp_rot, Kd_rot
	...
	double dAbsLimit[2] = { __AnkleWidth, 10.0 };
	...
	double dFMrol = Mfeet.rol / __AnkleWidth;
	double dFMpit = Mfeet.pit / fndAbsLimit(stStatePG.Ankle.W.Lfoot.pos.y - stStatePG.Ankle.W.Rfoot.pos.y, dAbsLimit) * fndActivate(fabs(stStatePG.Ankle.W.Lfoot.pos.y - stStatePG.Ankle.W.Rfoot.pos.y), __AnkleWidth, 'B');
	FootFTstab.fz = fndAbsWeight(dFMrol, dFMpit, 'B');
	// printf("%lf\t%lf\t%lf\n", dFMrol,  dFMpit, FootFTstab.fz); // test
	...
	MdzmpConVal.pos.ddx = -Mmdzmp.rol / __MUpper / dZcHonda - Kp_pos * MdzmpConVal.pos.x - Kd_pos * MdzmpConVal.pos.dx;
	MdzmpConVal.pos.ddy = Mmdzmp.pit / __MUpper / dZcHonda - Kp_pos * MdzmpConVal.pos.y - Kd_pos * MdzmpConVal.pos.dy;
	...
	stStateRef.Base.rot.pit += 0.0 * MdzmpConVal.rot.pit;
	stStateRef.Base.rot.rol += 0.0 * MdzmpConVal.rot.rol;
#endif
	for (int i = 0; i < 2; i++, nRek++) {
...
void fnvGRFC(double dParas[9]) { // Kf_z, Kp_z, Kd_z, Kc_z, Kdf_z, Kdfc_z, Kf_r, Kp_r
	...
	double dLimit_z[4] = { -0.02, 0.04, -10.0, 10.0 }, dLimit_z2nd[6] = { -0.02, 0.04, -10.0, 10.0, -2500.0, 2500.0 }, dLimit_r[4] = { -DccD2R(15.0), DccD2R(15.0), -10.0 * 6.0, 10.0 * 6.0 };
...
void fnvStepConNew(double dTswiTsup[2], double dStepLen) {
	...
	double dAmpx = 0.8, dAmpy = 0.8, dAmpz = 0.8, dSpeedAmp = 1.0;
...
void fnvRunConHonda() {
	double dFBCom = 1.0, dFBvCom = 1.0;
	#ifdef USE_SUPPOSCON
		dFBCom = 1.0 * 1.65;
		dFBvCom = 1.6 * 1.65;
	#endif
	double dSoftFootGain = 0.4; // 0.1;
	double dHardSpringGain = 1.0; // 10.0;
	double dSoftFootGainz = 1.0;
	double dHardSpringGainz = 1.0;
	double dFootGeom_in[4] = { /*forw*/0.13, /*back*/0.095, /*iner*/0.05, /*outer*/0.075 }; // { /*forw*/0.13, /*back*/0.095, /*iner*/0.05, /*outer*/0.075 };
	...
	double dParas_GRFC[9] = { /*Kf_z*/dSoftFootGainz * 0.025, /*Kp_z*/dHardSpringGainz * 100.0, /*Kd_z*/80.0, /*Kvc_z*/ 0.6 * 5.0, /*Kac_z*/ 0.6 * 0.005, /*Kdf_z*/ 0.35 * 0.0002, /*Kdfc_z*/ 0.4 * 0.00012, /*Kf_r*/dSoftFootGain * 0.06, /*Kp_r*/dHardSpringGain * 2.0  }; 
	double dGrtBias[2] = { /*dGrtBias_pit*/-0.0 * 2.0, /*dGrtBias_pit*/0.0}; 
	double dParasCompliance[2] = { /*Kf*/ 0.002, /*Kp*/ 104.0 };
	double dTswiTsup[2] = { /*Tswi*/ 0.285, /*Tsup*/ 0.315 }; // 0.3 
