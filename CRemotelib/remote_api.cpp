...
void CRobotConfig::fnvDataRecording() {
	...
	for (int i = 0; i < 6; i++, nDataStartTemp++) 
		SetLogName("R_Torque", "num", i), fptDataBuff[nDataStartTemp] = Real_LegTorque[1][i + 1];
	for (int i = 0; i < 6; i++, nDataStartTemp++)
		SetLogName("L_Torque", "num", i), fptDataBuff[nDataStartTemp] = Real_LegTorque[2][i + 1];
...
int main() {
	...
	double dPushTime_1[2] = { 2.5, 2.6 }; double dPushForce_1[3] = { 0.0, -0.0 * 100.0, 0.0 }; 
